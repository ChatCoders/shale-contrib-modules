 ...
 * to os_akillchan except that it KLINE's
 * instead of utilizing GLINE's/AKILL's.
 *
 * Default KLINE time is 1 week (604800 seconds)
 *
 */
...
command_t os_listklinechans = { "LISTKLINECHAN", "Lists active KLINE channels.", PRIV_MASS_AKILL, 1, os_cmd_listklinechans, { .path = "contrib/listklinechans" } };
...
static void klinechan_check_join(hook_channel_joinpart_t *hdata)
{
    ...
    if (metadata_find(mc, "private:klinechan:closer")) {
        ...
        if (has_priv_user(cu->user, PRIV_JOIN_STAFFONLY))
            ...
            char buf[BUFSIZE];
            snprintf(buf, sizeof(buf), "Not adding KLINE for *@%s due to klinechan %s (user %s!%s@%s is exempt)",
                     khost, cu->chan->name,
                     cu->user->nick, cu->user->user, cu->user->host);
            wallops_sts(buf);
        } else {
            snprintf(reason, sizeof reason, "Joined KLINE channel %s",
                     cu->chan->name);
            slog(LG_INFO, "klinechan_check_join(): Adding KLINE for \2*@%s\2 (user \2%s!%s@%s\2 joined \2%s\2)",
                 khost, cu->user->nick,
                 cu->user->user, cu->user->host,
                 cu->chan->name);

            kline_sts("*", "*", khost, 604800, reason);
        }
    }
}
...
static void os_cmd_klinechan(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    if (!strcasecmp(action, "ON")) {
        ...
        if (metadata_find(mc, "private:klinechan:closer")) {
            command_fail(si, fault_nochange, "\2%s\2 is already a KLINECHAN channel.", target);
            return;
        }
        ...
        command_success_nodata(si, "KLINECHAN enabled for \2%s\2.", target);
    } else if (!strcasecmp(action, "OFF")) {
        if (!metadata_find(mc, "private:klinechan:closer")) {
            command_fail(si, fault_nochange, "\2%s\2 is not a KLINECHAN.", target);
            return;
        }
        ...
        command_success_nodata(si, "KLINECHAN disabled for \2%s\2.", target);
    } else {
...
static void os_cmd_listklinechans(sourceinfo_t *si, int parc, char *parv[])
{
    ...
    if (matches == 0)
        command_success_nodata(si, _("No KLINE channels matched pattern \2%s\2"), pattern);
    else
